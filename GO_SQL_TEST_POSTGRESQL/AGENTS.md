...
## SQL主導 + GORM最小利用の方針
- 原則は SQL 主導で業務ルールを表現し、ORM 記法への過剰な統一を避ける
...
- 一意制約、`ON CONFLICT`、条件付き INSERT/UPDATE で DB 側に整合性を寄せる
- 同時実行を前提に、1 ステートメントまたは明示トランザクションで完結させる
- 実装レビューでは「同時実行時に二重登録しないか」を必須確認とする
