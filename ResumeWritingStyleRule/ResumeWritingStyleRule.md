...
- 履歴書・職務経歴書の作成では、読みやすさと管理しやすさを優先する。
- このプロジェクトの Markdown は、媒体転記前の下書きとして扱う。
- タイトルや使用技術は、転職サイト側に直接入力する場合は本文に重複記載しなくてよい。
- 各ファイルは、背景・課題・対応・成果が簡潔に伝わる構成にする。
- Do not use markdown bold markers like `**...**` for emphasis.
- Do not use separator lines such as `---` as visual dividers in resume text.
