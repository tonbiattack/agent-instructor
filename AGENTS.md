...
- **複雑なクエリは生クエリを許容**: 集計、複雑なJOIN、サブクエリなどで GORM の表現が不自然または可読性を損なう場合は、無理に GORM を使わず `Raw` / `Exec` などの生クエリを使用する
...
- **複雑なクエリでは生クエリを選択する**: GORM で無理に組み立てず、SQL の方が明確で保守しやすい場合は生クエリを採用する
