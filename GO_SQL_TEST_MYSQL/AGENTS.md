# Go SQLテスト実装ガイド（MySQL）

## 目的
このガイドは、MySQL を対象に SQL ファイル分離、スキーマ投入、初期データ管理、Go 統合テストを進めるための実装方針を定義する。

## 基本方針
- SQL は Go コードに直書きせず `.sql` ファイルとして管理する
- DB スキーマの正本は共通 SQL に寄せ、MySQL 固有差分のみを分離する
- DB を使う検証は実 MySQL に対する統合テストで行う
...
- MySQL 差分: `sql/dialect/mysql/*.sql`
...
- 1回のSQLで効率よく取得・判定できる処理は、アプリ側で分割せずSQLで完結させる
- Go + GORM で実装する場合は、生クエリを `Raw` / `Exec` で実行する

## スキーマ・初期データ管理
- `schema.sql` は DDL の正本として扱う
- `data.sql` は開発用の最小初期データとして扱う
- テスト固有データは `data.sql` に混ぜず、各テストまたは `test/testdata/*.sql` で投入する
- MySQL でのみ必要な補正は `sql/dialect/mysql/schema_patch.sql` や `data_patch.sql` に切り出す
...
## チェックリスト
- [ ] 共通 SQL と MySQL 差分の置き場を分けた
- [ ] `schema.sql` / `data.sql` を正本として扱っている
- [ ] MySQL 固有差分のみ `sql/dialect/mysql` に閉じ込めた
