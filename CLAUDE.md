# Go テストファースト開発ガイド（GORM + MySQL + Cobra + Gin）
...
- Cobra コマンドの入出力を定義
- Gin API の HTTP 入出力（ステータスコード、レスポンスJSON）を定義
...
### 5. API はクリーンアーキテクチャで設計する
- `domain`: エンティティ・ドメインルール
- `usecase`: アプリケーション固有のユースケース
- `interface`: Gin ハンドラーや DTO などの入出力層
- `infrastructure`: GORM/MySQL などの永続化実装
- 依存方向は外側から内側のみ（`interface -> usecase -> domain`）
...
- **API実装（Gin）はクリーンアーキテクチャで構成する**
- **Gin ハンドラーにビジネスロジックを直接書かない**（usecase 経由で実行）
- **DBアクセスは実DB（テスト用DB）を使用**
- **集計・絞り込み・JOIN・存在判定は、まず SQL で解決を検討する**: SQL で自然に表現できる処理は、無理に GORM やアプリケーション側の後処理で組み立てない
- **複雑なクエリは生クエリを選択してよい**: GORM の表現が不自然、可読性が低い、または保守しにくい場合は、`Raw` / `Exec` などの生クエリを使用する
...
- **Gin API はクリーンアーキテクチャ（domain/usecase/interface/infrastructure）で分離する**
- **Gin ハンドラーではバリデーション・変換・レスポンス整形に責務を限定する**
- **クエリは可読性・保守性・実行計画を優先する**: 1クエリに過度に詰め込んで複雑化する場合は、責務ごとにクエリを分割してよい
- **アプリケーション側の後処理は限定的に行う**: SQL で不自然になる場合、または分割した方が全体の責務が明確になる場合に限る
- **1回のSQLで効率よく取得・判定できる処理はSQLで完結させる**: アプリ側の分岐や後処理で分解しない
- **Go + GORM では生クエリを優先的に使う**: 複雑処理は `Raw` / `Exec` で実行し、ORMチェインに無理に寄せない
